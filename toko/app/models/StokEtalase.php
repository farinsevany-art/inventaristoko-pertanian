...
class StokEtalase extends Database {
    ...
    public function getAll($filters = []) {
        ...
        if (!empty($filters['rak'])) {
            $rak = $this->escape($filters['rak']);
            $whereClauses[] = "se.rak = '$rak'";
        }
...
public function generateNewId() {
    ...
    public function insert($data) {
        ...
        $rak = $this->escape($data['rak'] ?? '');
        return $this->query("INSERT INTO stok_etalase (id_stok_etalase, total_stok_eta, rak) VALUES ('$id_stok_etalase', '$total', '$rak')");
    }
    ...
    public function update($id, $data) {
        ...
        $total = (int)$data['total_stok_eta'];
        $rak = $this->escape($data['rak'] ?? '');
        return $this->query("UPDATE stok_etalase SET total_stok_eta='$total', rak='$rak' WHERE id_stok_etalase='$id'");
    }
    ...
    // Get distinct rak values from stok_etalase
    public function getDistinctRak() {
        $res = $this->query("SELECT DISTINCT rak FROM stok_etalase WHERE rak IS NOT NULL AND rak != '' ORDER BY rak ASC");
        if (!$res) return [];
        $rows = $res->fetch_all(MYSQLI_ASSOC);
        $values = [];
        foreach ($rows as $r) {
            if (!empty($r['rak'])) $values[] = $r['rak'];
        }
        return $values;
    }
}
